...
class Challenge {
  ...
  Map<String, dynamic> toJson() {
    final json = {
      ...
      'type': type,
      'category':
          category ?? type, // Use category if available, fallback to type
      'points': rewardPoints, // Map rewardPoints to points for backend
      'coins': rewardCoins, // Map rewardCoins to coins for backend
      'isCompleted': isCompleted,
      'icon': icon ?? 'star', // Use icon if available, fallback to 'star'
      'difficulty': difficulty, // Include difficulty
    ...
    // Only include id if it's not empty (to avoid MongoDB ObjectId cast errors)
    if (id.isNotEmpty) {
      json['id'] = id;
    }
  ...
  factory Challenge.fromJson(Map<String, dynamic> json) {
    ...
    return Challenge(
      ...
      rewardCoins: _parseIntValue(json['coins'] ?? json['rewardCoins'], 0),
      rewardPoints: _parseIntValue(json['points'] ?? json['rewardPoints'], 0),
      targetValue: _parseIntValue(json['targetValue'], 1),
      currentValue: _parseIntValue(json['currentValue'], 0),
      isCompleted:
          json['isCompleted'] == null
              ? false
...
class GamificationService extends ChangeNotifier {
  ...
  // XP required to reach a given level (matches backend: 100 XP per level)
  int getXPForNextLevel(int level) {
    // Level 1 starts at 0 XP, level 2 at 100 XP, level 3 at 200 XP, etc.
    if (level <= 0) return 0;
    return 100 * level;
  }
  ...
  Future<void> _fetchChallengesFromBackend() async {
    ...
    debugPrint('💾 Using local challenges generation instead');
  ...
  Future<bool> _saveGamificationDataToBackend() async {
    try {
      ...
      if (_dailyChallenges.isNotEmpty) {
        debugPrint(
          '🎯 Challenge sync temporarily disabled - endpoint not available',
        );
  ...
  // Force refresh from backend (use after completing challenges)
  Future<void> refreshFromBackend() async {
    try {
      debugPrint('🔄 Force refreshing gamification data from backend...');
      final backendData = await _fetchGamificationDataFromBackend();
      if (backendData != null) {
        await _processBackendGamificationData(backendData);
        debugPrint('✅ Gamification data refreshed from backend');
        notifyListeners();
      } else {
        debugPrint('⚠️ No data received from backend');
      }
    } catch (e) {
      debugPrint('❌ Error refreshing from backend: $e');
    }
  }
  ...
  Future<bool> _recordActivityOnBackend() async {
    ...
    debugPrint('💾 Using local activity tracking instead');
